...
LIBECS_DM_CLASS( ODEeventStepper, AdaptiveDifferentialStepper )
{
public:
    LIBECS_DM_OBJECT( ODEeventStepper, Stepper )
    {
        ...
        PROPERTYSLOT( Real, Tolerance,
                      &ODEeventStepper::initializeTolerance,
                      &AdaptiveDifferentialStepper::getTolerance );

        PROPERTYSLOT( Real, AbsoluteToleranceFactor,
                      &ODEeventStepper::initializeAbsoluteToleranceFactor,
        ...
        PROPERTYSLOT( Integer, isStiff,
                      &ODEeventStepper::setIntegrationType,
                      &ODEeventStepper::getIntegrationType );
    ...
    ODEeventStepper( void );
    virtual ~ODEeventStepper( void );
    ...
    SET_METHOD( Real, EventStepInterval )
    {
        theEventStepInterval = value;
    }

    GET_METHOD( Real, EventStepInterval )
    {
        return theEventStepInterval;
    }
...
LIBECS_DM_INIT( ODEeventStepper, Stepper );

#define SQRT6 2.4494897427831779

ODEeventStepper::ODEeventStepper()
    : theSystemSize( -1 ),
...
void ODEeventStepper::initialize()
{
    AdaptiveDifferentialStepper::initialize();
    initializeStepper();
    normalMaxStepInterval = theMaxStepInterval;
    String fluxName;
    fluxName = "ExpressionFluxProcess";
    for( Integer i = 0;  i < theProcessVector.size(); ++i ){
        Process* const aProcess( theProcessVector[ i ] );
        if( aProcess->getPropertyInterface().getClassName() == fluxName ){
            theFluxVector.push_back( aProcess );    
        }else{
            theAssignementVector.push_back( aProcess ); 
        }  
    } 
}

void ODEeventStepper::fireFlux()
{
    for( Integer i = 0;  i < theFluxVector.size(); ++i ){
        Process* const aProcess( theFluxVector[ i ] );
        aProcess->fire();
    } 
}

void ODEeventStepper::fireAssignment()
{
    for( Integer i = 0;  i < theAssignementVector.size(); ++i ){
        Process* const aProcess( theAssignementVector[ i ] );
        aProcess->fire();
    } 
}

void ODEeventStepper::initializeStepper()
{
    isStiff = true;
    theStiffnessCounter = 0;
...
void ODEeventStepper::initializeRadauIIA( VariableVector::size_type aNewSystemSize )
{
    eta = 1.0;
...
void ODEeventStepper::calculateJacobian()
{
    Real aPerturbation;
...
void ODEeventStepper::setJacobianMatrix( Real const aStepInterval )
{
...
void ODEeventStepper::decompJacobianMatrix()
{
    int aSignNum;
...
Real ODEeventStepper::calculateJacobianNorm()
{
...
void ODEeventStepper::calculateRhs( Real const aStepInterval )
{
...
std::pair< bool, Real > ODEeventStepper::calculateRadauIIA( Real aStepInterval, Real aPreviousStepInterval )
{
...
Real ODEeventStepper::estimateLocalError( Real const aStepInterval )
{
    Real anError;
...
void ODEeventStepper::updateInternalStateRadauIIA( Real aStepInterval )
{
    if ( !theJacobianMatrix1 )
    {
...
bool ODEeventStepper::calculate( Real aStepInterval )
{
