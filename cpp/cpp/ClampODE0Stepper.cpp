...
LIBECS_DM_CLASS( ClampODE0Stepper, AdaptiveDifferentialStepper )
{
public:
    LIBECS_DM_OBJECT( ClampODE0Stepper, Stepper )
    {
        ...
        PROPERTYSLOT( Real, Tolerance,
                     &ClampODE0Stepper::initializeTolerance,
                     &AdaptiveDifferentialStepper::getTolerance );
        
        PROPERTYSLOT( Real, AbsoluteToleranceFactor,
                     &ClampODE0Stepper::initializeAbsoluteToleranceFactor,
        ...
        PROPERTYSLOT( Integer, isStiff,
                     &ClampODE0Stepper::setIntegrationType,
                     &ClampODE0Stepper::getIntegrationType );
    ...
    ClampODE0Stepper( void );
    virtual ~ClampODE0Stepper( void );
...
LIBECS_DM_INIT( ClampODE0Stepper, Stepper );

#define SQRT6 2.4494897427831779

ClampODE0Stepper::ClampODE0Stepper()
: theSystemSize( -1 ),
...
void ClampODE0Stepper::initialize()
{
...
void ClampODE0Stepper::initializeStepper()
{
    isStiff = true;
    theStiffnessCounter = 0;
...
void ClampODE0Stepper::initializeRadauIIA( VariableVector::size_type aNewSystemSize )
{
    eta = 1.0;
...
void ClampODE0Stepper::calculateJacobian()
{
    Real aPerturbation;
...
void ClampODE0Stepper::setJacobianMatrix( Real const aStepInterval )
{
...
void ClampODE0Stepper::decompJacobianMatrix()
{
    int aSignNum;
...
Real ClampODE0Stepper::calculateJacobianNorm()
{
...
void ClampODE0Stepper::calculateRhs( Real const aStepInterval )
{
...
std::pair< bool, Real > ClampODE0Stepper::calculateRadauIIA( Real aStepInterval, Real aPreviousStepInterval )
{
...
Real ClampODE0Stepper::estimateLocalError( Real const aStepInterval )
{
    Real anError;
...
void ClampODE0Stepper::updateInternalStateRadauIIA( Real aStepInterval )
{
    if ( !theJacobianMatrix1 )
    {
...
bool ClampODE0Stepper::calculate( Real aStepInterval )
{
...
void ClampODE0Stepper::updateInternalState( Real aStepInterval )
{
    ...
    if ( getCurrentTime() >= _nextOnset - a *  interval || _nextOnset - getCurrentTime() > interval - b * interval || getCurrentTime() < _firstOnset ) {
        setMaxStepInterval( minStepInterval );

    } 
    else{ 
        //std::cout << _t << std::endl;  
        setMaxStepInterval( maxStepInterval ); 
    }
    if ( getCurrentTime() >= _nextOffset ) {
        _nextOnset  += interval;
        _nextOffset += interval;
    }    
    
    
    //setNextTime( getCurrentTime() + 0.001 );
    if( getNextTime() - getCurrentTime() < minStepInterval * min ){
        setNextTime( getCurrentTime() + minStepInterval * min ); 
    }       
}
