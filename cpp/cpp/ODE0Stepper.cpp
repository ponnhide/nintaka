...
LIBECS_DM_CLASS( ODE0Stepper, AdaptiveDifferentialStepper )
{
public:
    LIBECS_DM_OBJECT( ODE0Stepper, Stepper )
    {
        ...
        PROPERTYSLOT( Integer, isStiff,
                     &ODE0Stepper::setIntegrationType,
                     &ODE0Stepper::getIntegrationType );
        ...
        PROPERTYSLOT_SET_GET( Real, m );
    }
    
    ODE0Stepper( void );
    virtual ~ODE0Stepper( void );
...
LIBECS_DM_INIT( ODE0Stepper, Stepper );

#define SQRT6 2.4494897427831779

ODE0Stepper::ODE0Stepper()
: theSystemSize( -1 ),
...
void ODE0Stepper::initialize()
{
    AdaptiveDifferentialStepper::initialize();
    initializeStepper();
}

void ODE0Stepper::initializeStepper()
{
    isStiff = true;
    theStiffnessCounter = 0;
...
void ODE0Stepper::initializeRadauIIA( VariableVector::size_type aNewSystemSize )
{
    eta = 1.0;
...
void ODE0Stepper::calculateJacobian()
{
    Real aPerturbation;
...
void ODE0Stepper::setJacobianMatrix( Real const aStepInterval )
{
...
void ODE0Stepper::decompJacobianMatrix()
{
    int aSignNum;
...
Real ODE0Stepper::calculateJacobianNorm()
{
...
void ODE0Stepper::calculateRhs( Real const aStepInterval )
{
...
std::pair< bool, Real > ODE0Stepper::calculateRadauIIA( Real aStepInterval, Real aPreviousStepInterval )
{
    ...
    if( aNewStepInterval <= minStepInterval && m == 1 ){
        aNewStepInterval = minStepInterval;
    } 
    ...
    else
    {
        ...
        if( aNewStepInterval <= minStepInterval && m == 1 ){
            aNewStepInterval = minStepInterval;
            return std::make_pair( true, aNewStepInterval );
        }else{
...
Real ODE0Stepper::estimateLocalError( Real const aStepInterval )
{
    Real anError;
...
void ODE0Stepper::updateInternalStateRadauIIA( Real aStepInterval )
{
    if ( !theJacobianMatrix1 )
    {
    ...
    for ( ;; )
    {
        ...
        if( aStepInterval <= minStepInterval && m == 1 ){
            std::cout << aStepInterval << std::endl;
            aStepInterval = minStepInterval;
            break;
        } 
...
bool ODE0Stepper::calculate( Real aStepInterval )
{
...
void ODE0Stepper::updateInternalState( Real aStepInterval )
{
    ...
    if( getNextTime() - getCurrentTime() <= minStepInterval && m == 1 ){
        setNextTime( getCurrentTime() + minStepInterval ); 
    }       
}
