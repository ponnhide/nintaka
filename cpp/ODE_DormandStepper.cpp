...
LIBECS_DM_CLASS( ODE_DormandStepper, AdaptiveDifferentialStepper )
{
public:
    LIBECS_DM_OBJECT( ODE_DormandStepper, Stepper )
    {
        ...
        PROPERTYSLOT( Real, Tolerance,
                      &ODE_DormandStepper::initializeTolerance,
                      &AdaptiveDifferentialStepper::getTolerance );

        PROPERTYSLOT( Real, AbsoluteToleranceFactor,
                      &ODE_DormandStepper::initializeAbsoluteToleranceFactor,
        ...
        PROPERTYSLOT( Integer, isStiff,
                      &ODE_DormandStepper::setIntegrationType,
                      &ODE_DormandStepper::getIntegrationType );
        ...
        PROPERTYSLOT_SET_GET( Real, RadauMinStepInterval );
        PROPERTYSLOT_SET_GET( Real, DormandMinStepInterval );
    }

    ODE_DormandStepper( void );
    virtual ~ODE_DormandStepper( void );
    ...
    SIMPLE_SET_GET_METHOD( Real, RadauMinStepInterval );
    SIMPLE_SET_GET_METHOD( Real, DormandMinStepInterval);

    void setIntegrationType( Integer value )
    {
        //std::cout<<"change:"<<isStiff<<std::endl;
        initializeStepper();
        //std::cout<<"changed:"<<isStiff<<std::endl;
    }
...
LIBECS_DM_INIT( ODE_DormandStepper, Stepper );

#define SQRT6 2.4494897427831779

ODE_DormandStepper::ODE_DormandStepper()
    : theSystemSize( -1 ),
...
void ODE_DormandStepper::initialize()
{
...
void ODE_DormandStepper::initializeStepper()
{
...
void ODE_DormandStepper::initializeRadauIIA( VariableVector::size_type aNewSystemSize )
{
    eta = 1.0;
...
void ODE_DormandStepper::calculateJacobian()
{
    Real aPerturbation;
...
void ODE_DormandStepper::setJacobianMatrix( Real const aStepInterval )
{
...
void ODE_DormandStepper::decompJacobianMatrix()
{
    int aSignNum;
...
Real ODE_DormandStepper::calculateJacobianNorm()
{
...
void ODE_DormandStepper::calculateRhs( Real const aStepInterval )
{
...
std::pair< bool, Real > ODE_DormandStepper::calculateRadauIIA( Real aStepInterval, Real aPreviousStepInterval )
{
...
Real ODE_DormandStepper::estimateLocalError( Real const aStepInterval )
{
    Real anError;
...
void ODE_DormandStepper::updateInternalStateRadauIIA( Real aStepInterval )
{
    if ( !theJacobianMatrix1  )
    {
    ...
    if(aStepInterval < RadauMinStepInterval){
        aStepInterval = RadauMinStepInterval;
        setCurrentTime( aCurrentTime );
        fireProcesses();
...
bool ODE_DormandStepper::calculate( Real aStepInterval )
{
    ...
    //std::cout<<getMaxErrorRatio()<<std::endl;
...
void ODE_DormandStepper::updateInternalState( Real aStepInterval )
{
    ...
    if ( isStiff )
        ...
        while ( !calculate( aStepInterval )  )
        {
            ...
            aStepInterval = aStepInterval * safety * std::pow( getMaxErrorRatio(),-1.0 / getOrder() );       
            //aStepInterval *= 0.99;
        }
        ...
        const Real aCurrentTime( getCurrentTime() );
        aStepInterval = judgeEvent( aStepInterval, aCurrentTime );
        //std::cout<<aStepInterval<<":"<<theRadauMinStepInterval<<std::endl;
        if(aStepInterval < DormandMinStepInterval){
            aStepInterval = DormandMinStepInterval;
            setCurrentTime( aCurrentTime );
            fireProcesses();
            setVariableVelocity( theTaylorSeries[0] );
            setTolerableStepInterval( aStepInterval );
        ...
        if ( getMaxErrorRatio() < 0.005 )
        {
            ...
            //std::cout<<aStepInterval<<":"<<aNewStepInterval<<std::endl;
        }   
        setNextStepInterval( aNewStepInterval );
        DifferentialStepper::updateInternalState( aStepInterval ); 
    }
