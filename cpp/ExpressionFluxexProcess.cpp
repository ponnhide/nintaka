...
LIBECS_DM_CLASS_MIXIN( ExpressionFluxexProcess, ContinuousProcess,
                       SingleExpressionProcessBase )
{
public:
    LIBECS_DM_OBJECT( ExpressionFluxexProcess, Process )
    {
        INHERIT_PROPERTIES( ContinuousProcess );
        INHERIT_PROPERTIES( _LIBECS_MIXIN_CLASS_ );
        CLASS_DESCRIPTION("ExpressionFluxexProcess is designed for easy and efficient representations of continuous flux rate equations.    \"Expression\" property accepts a plain text rate expression.    The expression must be evaluated to give a flux rate in number per second, not concentration per second.");
        PROPERTYSLOT_SET_GET( String, mathml );
    
    }

    ExpressionFluxexProcess()
    {
    ...
    virtual ~ExpressionFluxexProcess()
    {
        ; // do nothing
    }
...
LIBECS_DM_INIT( ExpressionFluxexProcess, Process );
