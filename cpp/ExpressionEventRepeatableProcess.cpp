...
LIBECS_DM_CLASS_MIXIN( ExpressionEventRepeatableProcess, Process,
                       ExpressionProcessBase )
{
public:

    LIBECS_DM_OBJECT( ExpressionEventRepeatableProcess, Process )
    {
        INHERIT_PROPERTIES( _LIBECS_MIXIN_CLASS_ );
        INHERIT_PROPERTIES( Process );
        PROPERTYSLOT_SET_GET( String, Trigger );
        PROPERTYSLOT_SET_GET( String, Delay );
        PROPERTYSLOT_SET_GET( String, RepeatFlag );
	...
	PROPERTYSLOT_LOAD_SAVE( Polymorph, EventAssignmentList,
                                &ExpressionEventRepeatableProcess::setEventAssignmentList,
                                &ExpressionEventRepeatableProcess::getEventAssignmentList,
                                &ExpressionEventRepeatableProcess::setEventAssignmentList,
                                &ExpressionEventRepeatableProcess::getEventAssignmentList );
    }

    ExpressionEventRepeatableProcess()
      : theFireTime( 0.0 ),
        theRepeatFlag( "true" ),         
        theFireFlag( "false" ), 
    ...
    virtual ~ExpressionEventRepeatableProcess()
    {
        // ; do nothing
    }
    ...
    SET_METHOD( libecs::String, RepeatFlag )
    {
        theRepeatFlag = value;
    }

    GET_METHOD( libecs::String, RepeatFlag )
    {
        return theRepeatFlag;
    }
    ...
    virtual void initialize()
    {
        ...
        fired          = 0;
        previousTime   = getModel()->getCurrentTime();
        THROW_EXCEPTION_INSIDE(InitializationFailed, "No variable references with non-zero coefficients exist");
    }

    virtual void fire()
    {         
        if( getModel()->getCurrentTime() > previousTime && theFireFlag == "true" && theRepeatFlag == "false"){  
            theTriggerFlag = "true";
            fired = 1;
        }else if( fired == 0 ){ 
            theFireFlag = "false";             
            if ( theTriggerFlag == "false" && theFireFlag == "false" )
            {
                ...
                {
                    theFireTime = getStepper()->getCurrentTime() + 
                                theDelayProcessor.execute();
                    theTriggerFlag = "true";
                }
            }
            
            if ( theFireTime <= getStepper()->getCurrentTime() && theTriggerFlag == "true" && theFireFlag == "false" )
            {
                theTriggerFlag = "false";
                theFireFlag    = "true";
...
protected:
    ...
    class ExpressionProcessor
    {
       public:
        ...
        ExpressionProcessor( ExpressionEventRepeatableProcess* thisProcess,
        ...
        void setThisProcess( ExpressionEventRepeatableProcess* thisProcess )
        {
      ...
      private:
        ...
        ExpressionEventRepeatableProcess* _thisProcess;
    };
