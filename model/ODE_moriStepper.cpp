...
LIBECS_DM_CLASS( ODE_moriStepper, AdaptiveDifferentialStepper )
{
public:
    LIBECS_DM_OBJECT( ODE_moriStepper, Stepper )
    {
        ...
        PROPERTYSLOT( Real, Tolerance,
                      &ODE_moriStepper::initializeTolerance,
                      &AdaptiveDifferentialStepper::getTolerance );

        PROPERTYSLOT( Real, AbsoluteToleranceFactor,
                      &ODE_moriStepper::initializeAbsoluteToleranceFactor,
        ...
        PROPERTYSLOT( Integer, isStiff,
                      &ODE_moriStepper::setIntegrationType,
                      &ODE_moriStepper::getIntegrationType );

        PROPERTYSLOT_SET_GET( Integer, CheckIntervalCount );
        PROPERTYSLOT_SET_GET( Integer, SwitchingCount );
    }

    ODE_moriStepper( void );
    virtual ~ODE_moriStepper( void );
...
LIBECS_DM_INIT( ODE_moriStepper, Stepper );

#define SQRT6 2.4494897427831779

ODE_moriStepper::ODE_moriStepper()
    : theSystemSize( -1 ),
...
void ODE_moriStepper::initialize()
{
    AdaptiveDifferentialStepper::initialize();
    initializeStepper();
}

void ODE_moriStepper::initializeStepper()
{
    //isStiff = true;  
    theStiffnessCounter = 0;
...
void ODE_moriStepper::initializeRadauIIA( VariableVector::size_type aNewSystemSize )
{
    eta = 1.0;
...
void ODE_moriStepper::calculateJacobian()
{
    Real aPerturbation;
...
void ODE_moriStepper::setJacobianMatrix( Real const aStepInterval )
{
...
void ODE_moriStepper::decompJacobianMatrix()
{
    int aSignNum;
...
Real ODE_moriStepper::calculateJacobianNorm()
{
...
void ODE_moriStepper::calculateRhs( Real const aStepInterval )
{
...
std::pair< bool, Real > ODE_moriStepper::calculateRadauIIA( Real aStepInterval, Real aPreviousStepInterval )
{
...
Real ODE_moriStepper::estimateLocalError( Real const aStepInterval )
{
    Real anError;
...
void ODE_moriStepper::updateInternalStateRadauIIA( Real aStepInterval )
{
    if ( !theJacobianMatrix1 )
    {
...
bool ODE_moriStepper::calculate( Real aStepInterval )
{
...
void ODE_moriStepper::updateInternalState( Real aStepInterval )
{
