(function (root, factory) {
	...
	function LineChart(opts) {
		...
		var lc = {
			...
			graph_height: opts.graph_height || 450,
		...
		function calculate_domain(lc) {
			if( d3.min(lc.all_series, function(s) { return s.values[s.values.length-1].x}) < xmin || xmax <  d3.max(lc.all_series, function(s) { return s.values[s.values.length-1].x }) ){ 
		    	    xmin = d3.min(lc.all_series, function(s) { return d3.min(s.values, function(c) { return c.x }); });
			    if(lc.all_series[0].values.length < lc.maxLength ){ 
			    	xmax = d3.max(lc.all_series, function(s) { return 2 * d3.max(s.values, function(c) { return c.x }); }); 
			    } else {
			        xmax = d3.max(lc.all_series, function(s) { return d3.max(s.values, function(c) { return c.x }); });
			    }
			    lc.x_scale.domain([
				xmin, 	
				xmax	
			...
			if( d3.min(lc.all_series, function(s) { return s.values[s.values.length-1].y}) < ymin || ymax <  d3.max(lc.all_series, function(s) { return s.values[s.values.length-1].y }) ){ 
			    ymin = d3.min(lc.all_series, function(s) { return d3.min(s.values, function(c) { return c.y }); });
			    ymax = d3.max(lc.all_series, function(s) { return d3.max(s.values, function(c) { return c.y }); }); 
			    lc.y_scale.domain([
				ymin,
				ymax
				    ]);
